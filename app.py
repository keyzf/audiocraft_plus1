            ...
            if not audio_path.name.endswith(".json"):
                ...
                with open(audio_path.name) as json_file:
                    ...
                    if 'texts' not in data:
                        return "No tags found. Either the file is not generated by MusicGen+ V1.2.8 and higher or the tags are corrupted."
                    prompts = str("Prompts: " + data['texts'])
                    duration = str("Duration: " + data['duration'])
                    overlap = str("Overlap: " + data['overlap'])
                    seed = str("Seed: " + data['seed'])
    ...
    while ind < prompt_amount:
        for ind2 in range(int(drag_cat[ind])):
            if not struc_prompt:
                ...
                texts.append("bpm " + str(bpm) + ", " + str(key) + " " + str(scale) + ", " + str(global_prompt) + ", " + str(text_cat[ind]))
        ind2 = 0
        ind = ind + 1
